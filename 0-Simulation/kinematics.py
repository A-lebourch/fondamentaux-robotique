...
def computeIKOrientedSimple(x, y, z, legID, extra_theta=0, verbose=False):
    print("computeIKOrientedSimple not implemented yet")
...
def computeIKNotOriented(x, y, z, legID, params, verbose=False):
    print("computeIKNotOriented not implemented yet")
...
"""Autre façon de faire le cercle
...
def segment(
    ...
    half_duration = 0.5 * duration
    P1 = np.array([segment_x1, segment_y1, segment_z1])
    P2 = np.array([segment_x2, segment_y2, segment_z2])
    t_lin = math.fmod(t, half_duration) / half_duration
    if math.fmod(t, duration) < half_duration:
        pos = P2 * t_lin + (1 - t_lin) * P1
    else:
        pos = P1 * t_lin + (1 - t_lin) * P2

    return computeIK(pos[0], pos[1], pos[2])

def main():
    print("Ecrivez vos tests ici")
